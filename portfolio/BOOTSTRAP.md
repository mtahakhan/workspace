...
**Read `portfolio/AGENT_NOTES.md` before Step 4** - it has the full rule set
...
Read the **"What This Does"** section of `portfolio/README.md` and present it
...
Check if `portfolio/.env` exists. **Never ask the user to paste their API key
into chat** - it would end up in the session transcript. Instead:

1. If `portfolio/.env` doesn't exist, copy `portfolio/.env.example` to
   `portfolio/.env` (the example file just has a placeholder, safe to copy)
2. Point the user to the "Getting a Finnhub API key" section of
   `portfolio/README.md` (free signup, no card required) and ask them to open
   `portfolio/.env` themselves and replace the placeholder with their real key
...
4. To check whether they've filled it in, only check for the *absence* of the
   placeholder text (e.g. `grep -q your_key_here portfolio/.env`) - don't
...
exactly, in this exact order. Do not skip the script and reason it out
yourself, even if you're confident.** A prior bootstrap run guessed tickers by
reasoning instead of running this script and got 7 out of 7 wrong - including
...
FIFO lots from a sort-order bug) - don't skip it just because the script ran
without errors.
...
follow `portfolio/tasks/price-fetch.md`" / `.../daily-analysis.md`) - the real
